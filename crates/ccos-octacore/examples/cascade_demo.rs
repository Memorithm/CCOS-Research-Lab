...
//! (`OllamaEmbedder`) and CCOS as the causal scope (the CCOS-side `CcosScope`
//! adapter, behind CCOS_EXTENDED's `octacore` feature), this is the validated
//! cascade. Run: `cargo run --release --example cascade_demo`
...
fn main() {
    ...
    println!(
        "Swap InMemoryScope for the CCOS-side CcosScope (CCOS_EXTENDED --features\n\
         octacore) and HashEmbedder for OllamaEmbedder to run the real cascade.\n\
         SLHAv2 (--features slha) visualises the KV-cache via octacore::slha::kv_cache_view.\n\
         See README.md."
    );
}
