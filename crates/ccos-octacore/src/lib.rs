...
//!   not semantically. The CCOS-side adapter (`CcosScope`, in CCOS_EXTENDED's
//!   `src/octacore_bridge.rs`) implements [`CausalScope`] behind the root
//!   `octacore` feature — CCOS depends on this crate, not the reverse.
...
/// is CCOS — see the `octacore` feature in CCOS_EXTENDED, which supplies the
/// `CcosScope` adapter implementing this trait.)
#[derive(Default)]
