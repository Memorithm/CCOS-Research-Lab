...
|---|---|---|
| **Causal / structural** | CCOS | `trait CausalScope` — the CCOS-side `CcosScope` adapter (CCOS_EXTENDED `--features octacore`, in `src/octacore_bridge.rs`) implements it; this crate no longer depends on CCOS |
