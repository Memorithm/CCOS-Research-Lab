use forge_core::domains::cuda_kernel::CudaKernelDomain;
...
fn main() {
    ...
    println!("== Campagne cuda_gemm :: Ollama {model} @ {endpoint} ==");

    let domain = CudaKernelDomain::new("/tmp/forge_campaign_cuda").with_llm(&endpoint, &model);
    let envu = |k: &str, d: u64| -> u64 {
        std::env::var(k)
            .ok()
    ...
    eprintln!(
        "[forge] campagne: generations={} population={} survivors={}",
    ...
    match Engine::new(domain, config).run() {
        Ok(report) => {
            ...
            println!(
                "\n--- front de Pareto final ({} candidats) ---",
            ...
            for (i, ind) in report.final_front.iter().enumerate() {
                ...
                let ptx = ind.score.objectives.get(1).copied().unwrap_or(f64::NAN);
                let hold = report
                    .final_front_holdout
                    .get(i)
                    .and_then(|o| o.as_ref())
                ...
                println!("  [{i}] latency_ns={lat:.0}  ptx={ptx:.0}  holdout={hold}");
            }
            if let Some(bl) = report.final_baseline.as_ref() {
                println!(
                    "  baseline  latency_ns={:.0}  ptx={:.0}",
                    bl.objectives.get(0).copied().unwrap_or(f64::NAN),
                    bl.objectives.get(1).copied().unwrap_or(f64::NAN)
                );
            }
            ...
            match elite {
                        ...
                        < baseline_lat =>
                {
                    ...
                    let ptx = e.score.objectives.get(1).copied().unwrap_or(f64::NAN);
                    let speedup = baseline_lat / lat;
                    let src = e.cand.repr();
                    let dir = std::path::Path::new("/tmp/forge_elite_cuda");
                    let _ = std::fs::create_dir_all(dir);
                    let _ = std::fs::write(dir.join("kernel.cu"), &src);
                    let mut manifest = String::new();
                    let _ = writeln!(manifest, "model = {model}");
                    let _ = writeln!(manifest, "domain = cuda_gemm");
                    let _ = writeln!(manifest, "latency_ns = {lat:.0}");
                    let _ = writeln!(manifest, "ptx_instruction_count = {ptx:.0}");
                    ...
                    println!();
                    println!(">>> ELITE (holdout-verifie) -> /tmp/forge_elite_cuda/kernel.cu");
                    println!("    {lat:.0} ns / {ptx:.0} ptx vs {baseline_lat:.0} baseline ({speedup:.2}x), {} octets", src.len());
                }
                _ => {
                    println!();
