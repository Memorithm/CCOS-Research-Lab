...
fn main() {
    ...
    match Engine::new(domain, config).run() {
        Ok(report) => {
            ...
            let baseline_params = report
                .final_baseline
                .as_ref()
                .and_then(|b| b.objectives.get(2).copied())
            ...
            let elite = report
                ...
                .filter(|(i, ind)| {
                    ...
                    let hold_ok = report
                        ...
                        .map(|sc| {
                            sc.valid
                                && sc.objectives.get(0).copied().unwrap_or(f64::INFINITY) <= L2_TOL
                        })
                        .unwrap_or(false);
            ...
            match elite {
                Some((_, e))
                    if e.score.objectives.get(2).copied().unwrap_or(f64::INFINITY)
                        < baseline_params =>
                {
