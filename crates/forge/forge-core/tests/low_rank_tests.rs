...
fn test_low_rank_baseline_compiles_and_runs() {
    ...
    let trial = Trial { generation: 0, seed: 100 };

    // Le candidat baseline doit passer la vérification
    let valid = domain.verify(&cand, &trial).expect("verify should not error");
    assert!(valid, "Baseline candidate should compile and run successfully");
...
fn test_low_rank_invalid_code_fails_verify() {
    ...
    let valid = domain.verify(&bad_cand, &trial).expect("verify should not error");
...
fn test_low_rank_measure_returns_three_objectives() {
    ...
    let domain = TensorTrainDomain::new(workspace);
    let cand = domain.seed(&mut rand::rngs::StdRng::seed_from_u64(42));
    let trial = Trial { generation: 0, seed: 200 };

    let objectives = domain.measure(&cand, &trial).expect("measure should succeed");
