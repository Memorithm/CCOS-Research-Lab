...
//! OctaSoma C + audit hash-chaîné + criticité §7), avec deux modes :
//!
...
//!
//! CCOS_EXTENDED (plan P3) : le pont audit CCOS (`CcosAudit`) vit désormais côté
//! CCOS (`src/rsi_bridge.rs`, feature `rsi` du crate racine). Cette démo utilise
//! le `HashChainLog` in-crate à la place — plus de dépendance vers `ccos`.
//!
//! ```text
//! cargo run --release --bin rsi-full --features "forge octasoma" -- [pas] [graine]
//! cargo run --release --bin rsi-full --features "forge octasoma" -- compare [pas] [graine]
//! ```

use rsi::{
    report, CognitiveState, Dims, ForgeMetaSearch, ForgeSubstrate, HashChainLog,
    IntelligenceSurface, MetaOptimizer, RSIAgent, RiskConfig, Rng, StabilityConfig, StepReport,
    Substrate,
};
...
fn integrated(seed: u64, s: &CognitiveState, sub: &Substrate, surf: &IntelligenceSurface) -> RSIAgent {
    ...
    .with_audit(Box::new(HashChainLog::new()))
