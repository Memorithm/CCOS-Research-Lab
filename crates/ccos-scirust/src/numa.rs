...
// CCOS_EXTENDED plan P4: this is one of the two audited `unsafe` zones of the
// crate (see the crate-root `#![deny(unsafe_code)]`). The unsafe here is the
// aligned allocator (`std::alloc`) plus, behind the default-off `numa` feature,
// the Linux NUMA policy (`libc` mmap/mbind/sched_setaffinity). Every `unsafe`
// block below carries a `// SAFETY:` justification.
#![allow(unsafe_code)]
...
// SAFETY: `AlignedBuffer` owns a raw heap allocation (no shared Rust lifetime).
// The handle is just a `NonNull` + `Layout` + `len`; moving it across threads
// moves only these metadata, not any shared mutable borrow. The pointed-to
// bytes are untyped content with no thread affinity, so `Send` is sound. `Sync`
// is sound because `&AlignedBuffer` only yields `&[u8]` (read) — no interior
// mutability, no shared `&mut` — so concurrent shared references are safe.
// Synchronized mutable access (e.g. `as_mut_slice` via `&mut`) is the caller's
// responsibility, exactly like `&mut [u8]`.
...
mod imp {
    ...
    // SAFETY: same rationale as `AlignedBuffer` — `NumaBuffer` owns an mmap'd
    // region (page-aligned). The handle is `NonNull` + `len` + `cap`; moving it
    // across threads moves only metadata. `&NumaBuffer` yields only `&[u8]`
    // (no interior mutability), so `Sync` is sound. Synchronized `&mut` access
    // (zero/as_mut_slice) is the caller's responsibility.
