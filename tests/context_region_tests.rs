...
use ccos_research_lab::context_region::ContextRegion;
use ccos_research_lab::event_log::{EventLog, GraphReconstructor};
...
fn regions_are_created_automatically() {
    ...
    let created = log
        ...
        .filter(|e| {
            matches!(
                e.payload,
                ccos_research_lab::event_log::EventPayload::RegionCreated { .. }
            )
        })
        .count();
    assert_eq!(created, 2);
